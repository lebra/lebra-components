...
export  class Tabs extends React.PureComponent{
  ...
  //idx: number
  shouldRenderTab = (idx) => {
    const { destroyInactiveTab, prerenderingSiblingsNumber = 0 } = this.props;
    const { minRenderIndex, maxRenderIndex, currentTab = 0 } = this.state;
  ...
  goToTab(index, force = false, newState={}) {
    ...
    const { tabs, onChange, prerenderingSiblingsNumber } = this.props;
