...
- 如果需要，尽量使用src/style/theme/default.less里面定义的公共样式变量。
- 组件样式调用，使用classnames模块，进行样式处理，使用className调用
- 所有组件默认类名命名以`lebra-``开头
- 组件使用clsPrefix为样式前缀,用户也可在组件上设置自定义的`clsPrefix="lebra-input"`
```javascript
const clsPrefix = 'lebra-input';
const class1 = {
    [`${clsPrefix}-item`]: true,
...
```
- 可提供多种颜色的组件，在写less文件时，颜色设置需提出公用的minxin方法例如Alert组件中设置多种颜色： @alert-styles-variant
...
class Example extends React.Component {
    ...
    render() {
        ...
        const localeObj = this.context.lebraLocale ? this.context.lebraLocale : i18n[locale];
        cosnt { ok, cancel } = locale.Example;
...
[rem编写开发参考](https://www.w3cplus.com/mobile/lib-flexible-for-html5-layout.html)
